...
public class HangfireJobExecutor : IJobExecutor
{
    ...
    public Task<bool> RegisterRecurringJobAsync(JobDefinitionEntity jobDefinition)
    {
        try
        {
            ...
            RecurringJob.AddOrUpdate(
                jobDefinition.Name,
                () => ExecuteJobInternal(jobDefinition.Type, jobDefinition.Method),
        ...
        {
            _logger.LogError(ex, "Failed to register recurring job {JobName}", jobDefinition.Name);
    ...
    public Task<string> EnqueueJobAsync(JobDefinitionEntity jobDefinition)
    {
        ...
        {
            _logger.LogError(ex, "Failed to enqueue job {JobName}", jobDefinition.Name);
