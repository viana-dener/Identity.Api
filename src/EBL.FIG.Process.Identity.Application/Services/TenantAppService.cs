...
public class TenantAppService : ITenantAppService
{
    ...
    public async Task<bool> CreateAsync(CreateTenantRequest request, CancellationToken ct)
    {
        ...
        var entity = new TenantEntity(request.Name, request.Description, request.Alias, request.UrlImage, request.Settings, request.Remarks, _currentUser.GetUserId());
    ...
    public async Task<bool> UpdateAsync(int id, UpdateTenantRequest request, CancellationToken ct)
    {
        ...
        entity.Update(request.Name, request.Description, request.Alias, request.UrlImage, request.Settings, request.Remarks, _currentUser.GetUserId());
    ...
    private async Task<bool> ProcessBulkItemsAsync(List<BulkUploadTenantItem> items, CancellationToken ct)
    {
        ...
        foreach (var item in items)
        {
            ...
            // Cria a entidade
            var entity = new TenantEntity(item.Name, item.Description, item.Alias, item.UrlImage, item.Settings, item.Remarks, _currentUser.GetUserId());
