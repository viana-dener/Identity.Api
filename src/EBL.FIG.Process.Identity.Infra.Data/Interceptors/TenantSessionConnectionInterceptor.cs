...
namespace EBL.FIG.Process.Identity.Infra.Data.Interceptors;
...
public class TenantSessionConnectionInterceptor : DbConnectionInterceptor
{
    ...
    private int? ResolvetenantId()
    {
        ...
        if (httpContext is null)
        {
            ...
            {
                _logger.LogDebug("[RLS] TenantId resolvido via IRequestTenantContext (sem HttpContext): {TenantId}",
                    _requestTenantContext.TenantId.Value);
                return _requestTenantContext.TenantId.Value;
            }

            _logger.LogDebug("[RLS] Sem HttpContext e sem IRequestTenantContext definido. TenantId não resolvido.");
            return null;
        }
        ...
        // 3. Usuário não autenticado: TenantId vem do IRequestTenantContext (ex: login, register)
        if (_requestTenantContext.TenantId.HasValue)
        {
