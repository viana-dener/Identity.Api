...
namespace EBL.FIG.Process.Identity.Api.i18n;
...
public class CurrentUserApiService : ICurrentUserService
{
    ...
    /// <summary>
    /// Retrieves the application identifier associated with the current HTTP request.
    /// </summary>
    /// <remarks>The method attempts to obtain the application identifier from the authenticated user's claims
    /// (using the claim types 'app_id', 'appId', or 'AppId'). If not found, it checks the 'x-app-id' HTTP header. If
    /// neither is present, a default value of 1 is returned, which is intended for development scenarios and may change
    /// in future implementations.</remarks>
    /// <returns>The application identifier as an integer. Returns 1 if the identifier cannot be determined from the current user
    /// claims or request headers.</returns>
    /// <exception cref="Exception">Thrown if the HTTP context is not available.</exception>
    public int GetAppId()
    {
        var httpContext = _httpContextAccessor.HttpContext;
        ...
        // Se não encontrou no token, busca no header
        if (httpContext.Request.Headers.TryGetValue("x-app-id", out var headerAppId))
        {
            return int.Parse(headerAppId.ToString());
        }
    ...
    /// <summary>
    /// Obtém o identificador do usuário para usar em AddedBy/ModifiedBy
