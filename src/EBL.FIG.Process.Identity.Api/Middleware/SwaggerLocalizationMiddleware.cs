...
namespace EBL.FIG.Process.Identity.Api.Middleware;
        ...
        if (path.Contains("/swagger"))
        {
            ...
            if (!string.IsNullOrEmpty(requestedCulture))
            {
                try
                {
                    ...
                    Log.Debug("?? [SwaggerLocalization] Culture set to: {Culture} for Swagger", requestedCulture);
                }
                catch (CultureNotFoundException ex)
                {
                    Log.Warning(ex, "?? [SwaggerLocalization] Invalid culture requested: {Culture}", requestedCulture);
                }
            }
        }
