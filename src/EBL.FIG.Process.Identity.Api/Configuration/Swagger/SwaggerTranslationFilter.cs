...
namespace EBL.FIG.Process.Identity.Api.Configuration.Swagger;
...
public class SwaggerTranslationFilter : IDocumentFilter
{
    ...
    public void Apply(OpenApiDocument swaggerDoc, DocumentFilterContext context)
    {
        try
        {
            ...
            Log.Debug("?? [SwaggerTranslation] Applying translations for culture: {Culture}", culture);
    ...
    private Dictionary<string, string>? LoadTranslations(string culture)
    {
        ...
        lock (_lock)
        {
            ...
            try
            {
                ...
                {
                    Log.Warning("?? [SwaggerTranslation] Localization folder not found: {Path}", localizationPath);
                ...
                foreach (var c in tryCultures)
                {
                    ...
                    if (files.Length == 0)
                    {
                        Log.Debug("?? [SwaggerTranslation] No files for pattern {Pattern} in {Path}", pattern, localizationPath);
                        continue;
                    }

                    Log.Debug("?? [SwaggerTranslation] Found {Count} files for culture token {CultureToken}", files.Length, c);
                ...
                if (merged.Count == 0)
                {
                    ...
                    {
                        Log.Warning("?? [SwaggerTranslation] No translations for {Culture}, trying fallback pt-PT", culture);
