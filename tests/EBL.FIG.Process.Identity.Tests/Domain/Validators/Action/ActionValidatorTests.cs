...
namespace EBL.FIG.Process.Identity.Tests.Domain.Validators.Action;

public class ActionValidatorTests
{
    private readonly Mock<ILocalizationService> _localizationMock = new();

    private ActionValidator CreateSut() => new(_localizationMock.Object);

    private static ActionEntity BuildAction(int id = 1, string name = "Action Test", string description = "Descrição", bool isActive = true, bool isDeleted = false)
    {
    ...
    [Fact(DisplayName = "ValidateForCreateAsync - Deve falhar quando Name excede 50 caracteres")]
    [Trait("Domain", "")]
    public async Task ValidateForCreateAsync_NameExcede50Chars_DeveRetornarErro()
    {
        var entity = BuildAction(name: new string('A', 51));
        var sut = CreateSut();
    ...
    {
        var entity = BuildAction(description: "");
        var sut = CreateSut();
    ...
    {
        var entity = BuildAction(description: new string('D', 256));
        var sut = CreateSut();
    ...
    {
        var entity = BuildAction(isDeleted: true);
        var sut = CreateSut();
    ...
    {
        var entity = BuildAction(isActive: false);
        var sut = CreateSut();
    ...
    {
        var entity = BuildAction(isActive: true);
        var sut = CreateSut();
    ...
    {
        var entity = BuildAction(isActive: false, isDeleted: true);
        var sut = CreateSut();
