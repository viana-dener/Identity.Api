...
public class ActionDomainServiceTests
{
    private readonly Mock<IActionDataRepository> _repoMock = new();
    private readonly Mock<IEntityDomainValidator<ActionEntity>> _validatorMock = new();
    ...
    private ActionDomainService CreateSut() => new(
        _repoMock.Object,
        _validatorMock.Object,
        _notifyMock.Object);

    private static ActionEntity BuildAction(int id = 1, string name = "Action Test")
    {
    ...
    [Fact(DisplayName = "GetAllAsync - Deve retornar lista de actions")]
    [Trait("Domain", "")]
    ...
    {
        var listPage = new ListPage<ActionEntity> { Items = [BuildAction(1)], TotalItems = 1, TotalPages = 1, PageNumber = 1, PageSize = 10 };
    ...
    {
        _repoMock.Setup(x => x.ExistsByNameAsync(TenantId, AppId, "Action Test", default)).ReturnsAsync(true);
    ...
    public async Task CreateAsync_ValidacaoFalha_DeveRetornarFalseENotificar()
    {
        ...
        _repoMock.Verify(x => x.CreateAsync(It.IsAny<ActionEntity>(), default), Times.Never);
    }
    ...
    public async Task UpdateAsync_ValidacaoFalha_DeveRetornarFalseENotificar()
    {
        ...
        Assert.False(result);
        _notifyMock.Verify(x => x.Add(It.IsAny<string>(), 400), Times.Once);
        _repoMock.Verify(x => x.UpdateAsync(It.IsAny<ActionEntity>(), default), Times.Never);
    }
    ...
    [Fact(DisplayName = "DeleteAsync - Deve deletar action com sucesso")]
    [Trait("Domain", "")]
